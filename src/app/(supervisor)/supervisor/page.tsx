...
export default function SupervisorPage() {
  ...
  useEffect(() => {
    ...
    // Polling a cada 4 segundos no painel do supervisor (pausa enquanto edita)
    const interval = setInterval(() => {
      if (!editingId) fetchDeliveries(true);
    }, 4000);

    return () => clearInterval(interval);
  }, [fetchDeliveries, editingId]);
                ...
                return (
                  <div key={item.id} className="p-4 hover:bg-slate-50 transition">
                    {isEditing ? (
                      ...
                      /* Modos de Exibição com Ações Rápidas */
